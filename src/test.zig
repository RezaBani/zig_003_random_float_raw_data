...
test "simple test" {
    ...
    try std.testing.expectEqual(main.DEFAULT_COUNT, try main.parseArgs(argumentSetOkEmpty));
    try std.testing.expectEqual(try std.fmt.parseUnsigned(usize, argumentSetOkOne[@intFromEnum(main.ArgumentsOrder.Count)], 10), try main.parseArgs(argumentSetOkOne));
    ...
    try std.testing.expectError(main.ArgsError.TooManyArgs, main.parseArgs(argumentSetBadExtra));
    {
        ...
        for (okArgs.items) |value| {
            const count = try main.parseArgs(value);
            try main.createFile(path, count);
